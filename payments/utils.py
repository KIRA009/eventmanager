...
def create_order(order_items, mode, user_details, seller):
	...
	for item in order_items:
		if item["type"] == "product":
			...
			# if (prod.cod_available and mode != 'code') or (prod.online_available and mode != 'online'):
			# 	raise AccessDenied(f'{prod.name} cannot be purchased in {mode} mode payment')
			original_seller = prod.user.seller
			...
			if original_seller.id != seller.id:
				resell_product = seller.resell_products.get(product=prod)
				if original_seller.id not in reseller_orders:
					reseller_orders[original_seller.id] = dict(
						items=[], amount=0, shipping_charges=0, resell_margin=0,
						has_free_delivery_above_amount=original_seller.has_free_delivery_above_amount,
						free_delivery_above_amount=original_seller.free_delivery_above_amount
					)
				reseller_orders[original_seller.id]['items'].append(
					(prod, item['meta_data'])
				)
				reseller_orders[original_seller.id]['amount'] += it.disc_price + resell_product.resell_margin
				reseller_orders[original_seller.id]['shipping_charges'] = max(
					reseller_orders[original_seller.id]['shipping_charges'], prod.shipping_charges
				)
				reseller_orders[original_seller.id]['resell_margin'] += resell_product.resell_margin
			else:
	...
	if seller_orders and seller.has_free_delivery_above_amount \
		and seller_orders['amount'] >= seller.free_delivery_above_amount:
		seller_orders['shipping_charges'] = 0
	for k, v in reseller_orders.items():
		if v['has_free_delivery_above_amount'] and v['amount'] >= v['free_delivery_above_amount']:
			v['shipping_charges'] = 0
	if mode == 'cod':
		order_id = str(uuid.uuid4())
	else:
		total_amount = seller_orders['amount'] + seller_orders['shipping_charges'] + sum(
			i['amount'] + i['shipping_charges'] for i in reseller_orders.values()
		)
